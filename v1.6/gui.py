...
class AmortizationApp(tk.Tk):
    ...
    def _create_input_widgets(self, parent_frame: ttk.Frame) -> None:
        ...
        ttk.Entry(parent_frame, textvariable=self.start_date_var).grid(row=2, column=1, sticky=tk.EW, padx=5, pady=5)

        ttk.Label(parent_frame, text="End Date").grid(row=2, column=2, sticky=tk.W, padx=5, pady=5)
        ttk.Entry(parent_frame, textvariable=self.end_date_var).grid(row=2, column=3, sticky=tk.EW, padx=5, pady=5)
